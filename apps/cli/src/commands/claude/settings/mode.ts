...
export default class ClaudeSettingsMode extends Command {
  static description =
    'Select the cache-aware routing mode: off (per-request, default), sticky (hysteresis: stay on the conversation model unless a switch beats the prompt-cache cost), or orchestrator (shadow-mode v2, computed not served).';

  static args = {
    mode: Args.string({
      description: 'off, sticky, or orchestrator',
      options: ['off', 'sticky', 'orchestrator'],
      required: true,
    }),
  };
